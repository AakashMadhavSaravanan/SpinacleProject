...
#************************************************************************************
#RAG CONCEPT IMPLEMENTED BY THARUN
...
def generate_recipe(image_path: str) -> str:
    ...
    img_path="B://Tharun//CAREER LEARNING//Internship and RESUME//SPINACLE//vegetables//potato2.jpg"
    # run inference on a local image
    response= CLIENT.infer(
        img_path, 
        model_id="yolov8-project-4qp9a/1?confidence=0.5"
    )
    ...
    for pred in response["predictions"]:
        ...
        total_objects.append(pred["class"])
        unique_objects = list(set(total_objects))
    #print("Unique objects",unique_objects)
    #print("Total objects",detected_objects)
    '''print("\n Detected Objects:")
    for obj in detected_objects:
        print(f" {obj['class']} | Confidence: {obj['confidence']} | BBox: {obj['bbox']}")
    '''
    # Loop Through Predictions and Draw Bounding Boxe

    os.environ["PINECONE_API_KEY"] = "pcsk_3nju1u_8bi7uBVsuGrA9GH5hurbkKk6PM3Pa2LQndv6SF2RgwtzYhfAo2D4hcaBV5jytNp"
    os.environ["PINECONE_HOST"] = "https://index1-d8ijbfz.svc.aped-4627-b74a.pinecone.io"
    # 1. Load PDF
    loader = PyPDFLoader("C://Users//sreemathi//Desktop//vegetable_recipes.pdf")
    documents = loader.load()

    '''for i, doc in enumerate(documents):
        print(" ".join(doc.page_content.strip().split()))'''
    # 2. Split text (optional but recommended for large PDFs)
    ...
    '''print("SPLIT DOCS")
    for j, docs in enumerate(split_docs):
        print(f"Chunk {j+1}")
        print(docs.page_content.strip())'''
    ...
    if index_name not in pc.list_indexes().names():
        ...
        vectorstore = PineconeVectorStore.from_documents(
            split_docs,  # Store CHUNKS, not full document
            embedding_model,
    ...
    api_key = "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"
    )
    def deepseek_api(prompt_template):
        ...
        try:
            ...
            response_text = completion.choices[0].message.content  # Extract response
            # Debugging
    ...
    '''result=vectorstore.similarity_search(unique_objects)[:4]
    for i, doc in enumerate(result):
        print(f"Document{i+1}, {doc.page_content}",end="\n ")
    '''
    #snippet 7
    prompt_template = PromptTemplate(
    input_variables=["context","question"],
    template="""You are a helpful AI cooking assistant that provides well-formatted answers to questions based on the given document. 
    If the information is not available, respond with the knowledge that you have"

    Given Context:
    {context}

    Please provide a properly formatted answer in natural language.
    ...
    def retrieve_context(inputs):
        docs = retriever1.invoke(inputs["question"])
        for i, doc in enumerate(docs):
            return " ".join(doc.page_content)
        #return "\n\n".join([f"Document {i+1}, {doc.page_content}" for i, doc in enumerate(docs)])

    question_to_context = RunnableLambda(retrieve_context)

    def format_prompt(inputs):
        return prompt_template.format(context=inputs["context"], question=inputs["question"])

    # Convert function to a LangChain Runnable
    format_prompt_runnable = RunnableLambda(format_prompt)

    deepseek_runnable = RunnableLambda(deepseek_api)

    parser = StrOutputParser()
    #formatted_template=prompt_template.format(context=question_to_context,question=RunnablePassthrough())
    # Create the full chain
    chain = ({"context": question_to_context, "question": RunnablePassthrough()} | format_prompt_runnable | deepseek_runnable |parser)

    question=' '.join(unique_objects) 
    full_question=f"Give me the full recipe of {question} "
    ...
    context_retrieved = retrieve_context({"question": full_question})
...
#CLOSE PROGRAM LOGIC FOR RAG CONCEPT
